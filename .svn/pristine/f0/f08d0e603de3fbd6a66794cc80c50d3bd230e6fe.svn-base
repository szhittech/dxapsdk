...
public class ApScanner implements IApTransDone{
    ...
    private OnApScanListener onApScanListener;
    ...
    public void setOnApScanListener(OnApScanListener onApScanListener) {
        this.onApScanListener = onApScanListener;
    }
                            ...
                            for (int i = list.size() - 1; i >= 0; i--) {
                                ...
                                if (filter == null || targetSsidd.toUpperCase().startsWith(filter.toUpperCase())) {//LSConfigure_
                                    ...
                                    if (onApScanListener != null) {
                                        onApScanListener.onScanResult(new ApDeviceBean(scanResult));
                                    }
                                }
                            }
                        }
                ...
                try {
                    ...
                    while (connecting) {
                        ...
                        if (!isConnAp) {
                            connApCount++;
                            if (onApScanListener != null) {
                                int cCount = onApScanListener.getConnApCount();
                                ClifeLog.w("uu== connApCount:" + connApCount + " ApCountTrigger:" + cCount + " ApCountTrigger:" + onApScanListener.isconnApCountTrigger());
                                if (connApCount >= cCount) {
                                    if (!onApScanListener.isconnApCountTrigger()) {
                                        onApScanListener.onConnDeviceApTimeout(context, ssid);
                                        onApScanListener.setIsconnApCountTrigger(true);
                                        aotuConnAp = false;
                                    }
                                }
                            }
                            ...
                            if (circle >= 7) {
                                //计数器清0
                                circle = 0;
                                if (onApScanListener == null || aotuConnAp) {
                        ...
                        if (isConnAp) {
                            ...
                            ClifeLog.e(circle + " uu== 未连上设备热点，当前连接WiFi:" + current + " 设备热点:" + mDeviceApWiFi + " ip:" + ((ip != null && ip.length > 0) ? ip[0] : "") + " connected:" + isConnAp + " connApCount:" + connApCount + " ApCountTrigger:" + (onApScanListener == null ? "" : onApScanListener.isconnApCountTrigger()));
                        }
                        bindTimes++;
                    ...
                    if (onApScanListener != null) {
                        onApScanListener.setIsconnApCountTrigger(false);
                    }
                }
            }
        });

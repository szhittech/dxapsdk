...
public class WifiAdmin {
    ...
    public List<WifiConfiguration> getConfiguration() {
        List<WifiConfiguration> li = mWifiManager.getConfiguredNetworks();// 得到配置好的网络信息
        if (li!=null&&li.size()>0){
            mWifiConfigurations = li;
            ClifeLog.i("uu== ====getConfiguration:" + li.size());
        }
    ...
    public boolean connect(String SSID, String Password, WifiConnect.WifiCipherType Type) {
        ...
        ClifeLog.i( "uu ====connect netID" + netID);
        // 断开连接
        mWifiManager.disconnect();
        // 重新连接
...
//            config.hiddenSSID = true;
    ...
    public boolean addNetwork(WifiConfiguration wcg) {
        ...
        System.out.println("===uuu==addNetwork= "+b);
        return b;
    }
    ...
    public boolean connectSpecificAP(ScanResult scan) {
        ...
        ClifeLog.i("uu== connectSpecificAP:"+networkInSupplicant);
    ...
    public boolean ConnectWifi(int wifiId) {
        ClifeLog.i("===uuuu++ConnectWifi++ "+(mWifiConfigurations==null?"bull":mWifiConfigurations.size()));
        for (int i = 0; i < mWifiConfigurations.size(); i++) {
            ...
            if (wifi.networkId == wifiId) {
                while (!(mWifiManager.enableNetwork(wifiId, true))) {// 激活该Id，建立连接
                    ClifeLog.i("ConnectWifi", String.valueOf(mWifiConfigurations.get(wifiId).status));// status:0--已经连接，1--不可连接，2--可以连接
                }
                return true;
            }
        }
    ...
    public int addWifiConfig(List<ScanResult> wifiList, String ssid, String pwd) {
        ...
        for (int i = 0; i < wifiList.size(); i++) {
            ...
            if (wifi.SSID.equals(ssid)) {
                ClifeLog.i("AddWifiConfig", "equals");

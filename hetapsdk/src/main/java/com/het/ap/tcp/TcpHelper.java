...
public class TcpHelper {
    ...
    public void connect(String HostIp, int HostPort) {
        try {
            ...
            socket = new Socket();
            readThread.start();
            ClifeLog.e("uu== 开始建立TCP " + HostIp + ":" + HostPort);
        } catch (Exception e) {
            ErrMsg = e.getMessage();
            ClifeLog.e("uu==TCP建立失败:" + ErrMsg);
        }
    }
    ...
    public void send(byte[] iBuf) {     //发送字节流指令
    ...
    public boolean isConnected() {
        ...
        ClifeLog.e("uu== tcp connect state:" + tcpConn);
        return socket.isConnected();
    }
                ...
                try {
                    if (connect()) {
                        if (inputStream.available() > 0) {
                            ...
                            ClifeLog.e(" uu== recv:" + ByteUtils.toHexString(recData));
